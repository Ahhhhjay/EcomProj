    ...
    public function apply()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($discountData && isset($discountData['discountRate'])) {
                $_SESSION['bookingData']['promoCode'] = $discountData['code'];

                $discountRate = doubleval($discountData['discountRate']); // Convert to numeric value

                $totalPrice = $_SESSION['bookingData']['totalPrice'];

                if ($discountRate != 0) {
                    $priceOFF = $discountRate / 100;
                    $discountAmount = $totalPrice * $priceOFF;
        ...
        } else if (isset($_GET['action']) && $_GET['action'] == 'denied') {
            $totalPrice = $_SESSION['bookingData']['totalPrice'];
